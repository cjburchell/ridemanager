...
type AthletesRoutesCall struct {
	service *AthletesService
	id      int64
	ops     map[string]interface{}
}

func (s *AthletesService) Routes(athleteId int64) *AthletesRoutesCall {
	return &AthletesRoutesCall{
		service: s,
		id:      athleteId,
		ops:     make(map[string]interface{}),
	}
...
func (c *AthletesRoutesCall) Page(page int) *AthletesRoutesCall {
	c.ops["page"] = page
	return c
}

func (c *AthletesRoutesCall) PerPage(perPage int) *AthletesRoutesCall {
	c.ops["per_page"] = perPage
	return c
}

func (c *AthletesRoutesCall) Do() ([]*Route, error) {
	data, err := c.service.client.run("GET", fmt.Sprintf("/athletes/%d/routes", c.id), c.ops)
	if err != nil {
		return nil, err
	}
