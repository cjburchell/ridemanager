...
type StarredSegmentsCall struct {
	service *SegmentsService
	ops     map[string]interface{}
}

func (s *SegmentsService) Starred() *StarredSegmentsCall {
	return &StarredSegmentsCall{
		service: s,
		ops:     make(map[string]interface{}),
	}
}

func (c *StarredSegmentsCall) Page(page int) *StarredSegmentsCall {
	c.ops["page"] = page
	return c
}

func (c *StarredSegmentsCall) PerPage(perPage int) *StarredSegmentsCall {
	c.ops["per_page"] = perPage
	return c
}

func (c *StarredSegmentsCall) Do() ([]*SegmentSummary, error) {
	data, err := c.service.client.run("GET", "/segments/starred", c.ops)
	if err != nil {
		return nil, err
	}

	efforts := make([]*SegmentSummary, 0)
	err = json.Unmarshal(data, &efforts)
	if err != nil {
		return nil, err
	}
